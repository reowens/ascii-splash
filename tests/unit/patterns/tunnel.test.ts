...
describe('TunnelPattern', () => {
  ...
  beforeEach(() => {
    ...
    pattern = new TunnelPattern(theme, new Mulberry32(42));
  ...
  describe('Constructor and Configuration', () => {
    ...
    test('should accept custom config', () => {
      const customPattern = new TunnelPattern(theme, new Mulberry32(42), {
        shape: 'square',
        ringCount: 50,
        speed: 3.0,
        particleCount: 100,
  ...
  describe('Rendering', () => {
    test('should render tunnel with rings', () => {
      ...
      const hasContent = buffer.some(row => row.some(cell => cell.char !== ' '));
    ...
    test('should use depth characters', () => {
      ...
      const usedChars = new Set(
        buffer
          .flat()
          .filter(
            cell => cell.char !== ' ' && cell.char !== '+' && cell.char !== '-' && cell.char !== '='
          )
          .map(cell => cell.char)
      );
    ...
    test('should render streaming particles', () => {
      ...
      const particleChars = ['●', '○', '·'];
      const hasParticles = buffer.flat().some(cell => particleChars.includes(cell.char));
    ...
    test('should render speed lines', () => {
      ...
      const fastPattern = new TunnelPattern(theme, new Mulberry32(42), {
        speed: 5.0,
        speedLineCount: 30,
      ...
      for (let t = 0; t < 10 && !hasSpeedLines; t++) {
        ...
        hasSpeedLines = testBuffer.flat().some(cell => speedLineChars.includes(cell.char));
      }
    ...
    test('should rotate rings over time', () => {
      const rotatingPattern = new TunnelPattern(theme, new Mulberry32(42), { rotationSpeed: 0.5 });
  ...
  describe('Tunnel Shapes', () => {
    test('should render circle tunnel', () => {
      const circlePattern = new TunnelPattern(theme, new Mulberry32(42), { shape: 'circle' });
      expect(() => circlePattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should render square tunnel', () => {
      const squarePattern = new TunnelPattern(theme, new Mulberry32(42), { shape: 'square' });
      expect(() => squarePattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should render hexagon tunnel', () => {
      const hexPattern = new TunnelPattern(theme, new Mulberry32(42), { shape: 'hexagon' });
      expect(() => hexPattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should render star tunnel', () => {
      const starPattern = new TunnelPattern(theme, new Mulberry32(42), { shape: 'star' });
  ...
  describe('Metrics', () => {
    ...
    test('should return correct ring count', () => {
      const customPattern = new TunnelPattern(theme, new Mulberry32(42), { ringCount: 50 });
      const metrics = customPattern.getMetrics();
    ...
    test('should return correct particle count', () => {
      const customPattern = new TunnelPattern(theme, new Mulberry32(42), { particleCount: 100 });
      const metrics = customPattern.getMetrics();
  ...
  describe('Edge Cases', () => {
    test('should handle zero ring count', () => {
      const zeroPattern = new TunnelPattern(theme, new Mulberry32(42), { ringCount: 0 });
    ...
    test('should handle extreme ring count', () => {
      const extremePattern = new TunnelPattern(theme, new Mulberry32(42), { ringCount: 100 });

      expect(() => extremePattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should handle zero speed', () => {
      const staticPattern = new TunnelPattern(theme, new Mulberry32(42), {
        speed: 0,
        rotationSpeed: 0,
        turbulence: 0,
      });
    ...
    test('should handle negative speed', () => {
      const reversePattern = new TunnelPattern(theme, new Mulberry32(42), { speed: -1.0 });

      expect(() => reversePattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should handle very high speed', () => {
      const fastPattern = new TunnelPattern(theme, new Mulberry32(42), { speed: 10.0 });

      expect(() => fastPattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should handle zero particle count', () => {
      const emptyPattern = new TunnelPattern(theme, new Mulberry32(42), { particleCount: 0 });
    ...
    test('should handle very high particle count', () => {
      const densePattern = new TunnelPattern(theme, new Mulberry32(42), { particleCount: 300 });

      expect(() => densePattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should handle zero turbulence', () => {
      const calmPattern = new TunnelPattern(theme, new Mulberry32(42), { turbulence: 0 });

      expect(() => calmPattern.render(buffer, 1000, size)).not.toThrow();
    });

    test('should handle high turbulence', () => {
      const chaoticPattern = new TunnelPattern(theme, new Mulberry32(42), { turbulence: 1.0 });
