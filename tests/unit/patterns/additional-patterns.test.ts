...
describe('Additional Pattern Tests', () => {
  describe('MatrixPattern', () => {
    ...
    // v0.4.0 Phase 3: Matrix is naturally sparse — only writes column cells,
    // so it composes over a photo background without needing a transparentBg
    // flag. This test pins that invariant so future changes don't silently
    // make Matrix dense.
    describe('Phase 3 layered-overlay compatibility', () => {
      it('leaves non-column cells untouched (preserves photo backgrounds)', () => {
        const size = createMockSize(60, 20);
        const buf = createMockBuffer(size.width, size.height);
        // Pre-fill with a sentinel that Matrix would never emit (its charsets
        // are katakana / numbers / binary / sigils — none use '_').
        for (let y = 0; y < size.height; y++) {
          for (let x = 0; x < size.width; x++) buf[y][x] = { char: '_' };
        }
        // Run several frames to give columns time to advance.
        for (let i = 0; i < 5; i++) pattern.render(buf, i * 33, size);

        // Most cells should still be the sentinel — Matrix paints columns
        // sparsely, leaving the rest of the buffer alone.
        let preservedSentinel = 0;
        for (let y = 0; y < size.height; y++) {
          for (let x = 0; x < size.width; x++) {
            if (buf[y][x].char === '_') preservedSentinel++;
          }
        }
        // Loose threshold: at least 25% of cells untouched. Matrix's actual
        // column count tops out well below 100% coverage even under high
        // density, but density and speed vary across presets.
        expect(preservedSentinel).toBeGreaterThan(size.width * size.height * 0.25);
      });
    });
  });
