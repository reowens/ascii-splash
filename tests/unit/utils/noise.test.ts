...
describe('Noise Utils', () => {
  describe('PerlinNoise', () => {
    ...
    describe('octaveNoise2D', () => {
      ...
      it('returns value between -1 and 1', () => {
        for (let i = 0; i < 100; i++) {
          const value = noise.octaveNoise2D(Math.random() * 100, Math.random() * 100, 4, 0.5);
          expect(value).toBeGreaterThanOrEqual(-1);
          expect(value).toBeLessThanOrEqual(1);
        }
      });
      ...
      it('adds detail with more octaves', () => {
        ...
        // (probabilistic but should hold true with the seed)
        expect(var4).toBeGreaterThan(0);
        expect(var1).toBeGreaterThanOrEqual(0);
      });
    });
  });
  ...
  describe('turbulence', () => {
    it('returns positive value', () => {
      for (let i = 0; i < 50; i++) {
        const value = turbulence(Math.random() * 100, Math.random() * 100, 32);
