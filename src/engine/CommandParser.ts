...
export type CommandType =
  'preset' | 'favorite' | 'saveFavorite' | 'pattern' | 'theme' | 'special' | 'combination';
...
export interface ParsedCommand {
  ...
  args: {
    ...
    patternPreset?: number; // For pattern+preset combo (e.g., 0p3.5)
    ...
    // Special
    specialCmd?: string;
    specialArg?: string; // For commands with arguments (e.g., 0!5)
...
export class CommandParser {
   ...
   */
  private parseCombination(cleanCmd: string, originalCmd: string): ParsedCommand | null {
   ...
   */
  private parseSingle(cleanCmd: string, originalCmd: string): ParsedCommand | null {
   ...
   */
  private parsePatternCommand(rest: string, originalCmd: string): ParsedCommand | null {
    if (!rest) {
    ...
    if (dotIndex > 0) {
      ...
      const patternId = /^\d+$/.test(patternPart) ? parseInt(patternPart, 10) : patternPart;

      const presetNumber = /^\d+$/.test(presetPart) ? parseInt(presetPart, 10) : undefined;
    ...
    const patternId = /^\d+$/.test(rest) ? parseInt(rest, 10) : rest;
   ...
   */
  private parseThemeCommand(rest: string, originalCmd: string): ParsedCommand | null {
    if (!rest) {
    ...
    const themeId = /^\d+$/.test(rest) ? parseInt(rest, 10) : rest;
   ...
   */
  private parseSpecialCommand(cleanCmd: string, originalCmd: string): ParsedCommand | null {
