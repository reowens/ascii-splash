...
export class FireworksPattern implements Pattern {
  ...
  private getShapedBurstParams(
    index: number,
    total: number,
    shape: 'circle' | 'ring' | 'heart' | 'star' | 'random'
  ): { angle: number; speedMult: number } {
    const t = index / total;
  ...
  private explode(firework: Firework, depth = 0): void {
        ...
        for (let j = fw.particles.length - 1; j >= 0; j--) {
          ...
          if (p.type === 'normal') {
            const trailLength = Math.max(
              2,
              Math.floor(this.config.trailLength * Math.pow(0.5, p.depth))
            );
          ...
          if (px >= 0 && px < width && py >= 0 && py < height) {
            ...
            if (p.type === 'sparkle') {
              ...
              color = {
                r: Math.floor(
                  rainbowColor.r * blendFactor +
                    fw.burstColor.r * (1 - blendFactor) * p.life * intensityScale
                ),
                g: Math.floor(
                  rainbowColor.g * blendFactor +
                    fw.burstColor.g * (1 - blendFactor) * p.life * intensityScale
                ),
                b: Math.floor(
                  rainbowColor.b * blendFactor +
                    fw.burstColor.b * (1 - blendFactor) * p.life * intensityScale
                ),
              };
            }
            ...
            if (tx >= 0 && tx < width && ty >= 0 && ty < height) {
              ...
              if (trailLife > 0.1) {
                ...
                buffer[ty][tx] = {
                  ...
                  color: {
                    r: Math.floor(
                      rainbowColor.r * blendFactor + fw.burstColor.r * (1 - blendFactor) * trailLife
                    ),
                    g: Math.floor(
                      rainbowColor.g * blendFactor + fw.burstColor.g * (1 - blendFactor) * trailLife
                    ),
                    b: Math.floor(
                      rainbowColor.b * blendFactor + fw.burstColor.b * (1 - blendFactor) * trailLife
                    ),
                  },
                };
              }
