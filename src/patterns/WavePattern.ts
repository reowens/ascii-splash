    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        for (let layer = 0; layer < layers; layer++) {
          ...
          const wave2 =
            Math.sin(x * layerFreq * 1.3 + time * layerSpeed * 0.0008) * (layerAmp * 0.5);
        ...
        if (this.config.foamEnabled && intensity < 0.5) {
          ...
          if (normalizedWave > this.config.foamThreshold) {
            ...
            if (foamNoise < this.config.foamDensity) {
              ...
              const foamIndex = Math.floor(
                (foamNoise * this.foamChars.length) / this.config.foamDensity
              );
  ...
  getMetrics(): Record<string, number> {
    ...
    const avgRippleAge =
      this.ripples.length > 0
        ? this.ripples.reduce((sum, r) => sum + (this.currentTime - r.time), 0) /
          this.ripples.length
        : 0;
